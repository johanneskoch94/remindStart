run <- function() {
  ...
  if (cfg$gms$CES_parameters == "load") {
    ...
    for (cal_itr in 1:cfg$gms$c_CES_calibration_iterations) {
      ...
      system(paste0(
        "sed -i 's/^\\(\\$setglobal c_CES_calibration_iteration ",
        "\\).*/\\1", cal_itr, "/' full.gms"
      ))

      system(paste0(
        cfg$gamsv, " full.gms -errmsg=1 -a=", cfg$action,
        " -ps=0 -pw=185 -gdxcompress=1 -logoption=", cfg$logoption
      ))

      # If GAMS found a solution
      if (file.exists("fulldata.gdx") && file.info("fulldata.gdx")$mtime > fulldataMTime) {
        ...
        fulldataMTime <- file.info("fulldata.gdx")$mtime
      } else {
        break
      }
    }
  ...
  gamsRunTime <- timeGAMSEnd - timeGAMSStart
  cat(
    "\nEnding REMIND, time is", format(Sys.time(), "%H:%M:%S"),
    "\n################\n\nREMIND run finished in", round(as.numeric(gamsRunTime), 2), units(gamsRunTime), "\n\n"
  )
