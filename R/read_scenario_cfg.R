read_scenario_cfg <- function(configFile, default.cfg = file.path(dirname(configFile), "default.cfg")) {
  ...
  if (length(grep("\\.", rownames(scenarios))) > 0) {
    stop(paste0("These titles contain dots: ",
  ...
  if (length(grep("_$", rownames(scenarios))) > 0) {
    stop(paste0("These titles end with _: ",
  ...
  if (any(nchar(rownames(scenarios)) > 75)) {
    stop(paste0("These have more than 75 characters ",
  ...
  for (iscen in scenarios[,1]) {
    ...
    path_gdx_list <- c("path_gdx", "path_gdx_ref", "path_gdx_refpolicycost", "path_gdx_bau", "path_gdx_carbonprice")
    if (any(iscen %in% settings[iscen, path_gdx_list])) {
      stop("Self-reference: ", iscen , " refers to itself in a path_gdx... column.")
    }
    for (path_to_gdx in path_gdx_list) {
      if (!is.na(settings[iscen, path_to_gdx]) & ! settings[iscen, path_to_gdx] %in% row.names(scenarios)) {
        if (! stringr::str_sub(settings[iscen, path_to_gdx], -4, -1) == ".gdx") {
          ...
          dirs <- Sys.glob(file.path(paste0("./output/",settings[iscen, path_to_gdx],"*/fulldata.gdx")))
          # if path_to_gdx cell content exactly matches folder name, use this one
          if (paste0("./output/",settings[iscen, path_to_gdx],"/fulldata.gdx") %in% dirs) {
            message(paste0("   For ", path_to_gdx, " = ", settings[iscen, path_to_gdx], ", a folder with fulldata.gdx was found."))
            settings[iscen, path_to_gdx] <- paste0("./output/",settings[iscen, path_to_gdx],"/fulldata.gdx")
          } else {
            ...
            dirs <- dirs[unlist(lapply(dirs, didremindfinish)) &
                           grepl(paste0(settings[iscen, path_to_gdx],"_?", datetimepattern, "/fulldata.gdx"), dirs)]
            ...
            if(length(dirs) > 0 && ! all(is.na(dirs))) {
              latest_fulldata <- lapply(dirs, stringr::str_sub, -32, -14) %>%
                ...
                as.numeric %>%
                which.max
              message(paste0("   Use newest normally completed run for ", path_to_gdx,
                             " = ", settings[iscen, path_to_gdx], ":\n     ",
                             stringr::str_sub(dirs[latest_fulldata], 10, -14)))
              settings[iscen, path_to_gdx] <- dirs[latest_fulldata]
            }
          }
        }
        # if the above has not created a path to a valid gdx, stop
        if (!file.exists(settings[iscen, path_to_gdx])){
          stop(paste0(
            "Can't find a gdx specified as ",
            settings[iscen, path_to_gdx], " in column ", path_to_gdx,
  ...
  while (length(ordered_scenarios) < dim(scenarios)[1]) {
    next_up <- scenarios %>%
      dplyr::filter(
        ...
        dplyr::if_all(dplyr::all_of(path_gdx_list), ~ is.na(.x) | .x %in% ordered_scenarios)
      ) %>%
      row.names()
